package rrn.mobile.gm2gpt;

import org.json.JSONException;
import org.json.JSONObject;

public class Maps {
    private String kegiatan;
    private String lokasi;
    private String imageUrl; // Updated field name to match JSON data

    // Default constructor
    public Maps() {
    }

    // Constructor for initialization from JSON
    public Maps(JSONObject jsonObject) throws JSONException {
        this.kegiatan = jsonObject.optString("kegiatan", "");
        this.lokasi = jsonObject.optString("lokasi", "");
        this.imageUrl = jsonObject.optString("imageUrl", ""); // Ensure compatibility with JSON key "imageUrl"
    }

    // Constructor with parameters
    public Maps(String kegiatan, String lokasi, String imageUrl) {
        this.kegiatan = kegiatan;
        this.lokasi = lokasi;
        this.imageUrl = imageUrl;
    }

    // Getter and Setter methods
    public String getKegiatan() {
        return kegiatan;
    }

    public void setKegiatan(String kegiatan) {
        this.kegiatan = kegiatan;
    }

    public String getLokasi() {
        return lokasi;
    }

    public void setLokasi(String lokasi) {
        this.lokasi = lokasi;
    }

    public String getImageUrl() {
        return imageUrl;
    }

    public void setImageUrl(String imageUrl) {
        this.imageUrl = imageUrl;
    }

    // Method to create a Maps object from JSON
    public static Maps fromJson(JSONObject jsonObject) {
        Maps map = new Maps();
        try {
            map.setKegiatan(jsonObject.getString("kegiatan"));
            map.setLokasi(jsonObject.getString("lokasi"));
            map.setImageUrl(jsonObject.getString("imageUrl")); // Use "imageUrl" key to match JSON data
        } catch (JSONException e) {
            e.printStackTrace();
        }
        return map;
    }
}
